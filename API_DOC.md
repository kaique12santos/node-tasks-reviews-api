...
Este documento descreve os recursos disponíveis na API, integrando o **Sistema de Gestão de Tarefas (API 1)** e o **Sistema de Avaliações (API 2)**.

2.  **Tecnologia:** Node.js, SQLite, JWT, Bcrypt.
---
3.  **Arquitetura:** MVC + Service Layer + POO.
---
...
*(Nota: O campo `perfil` aceita "user" ou "admin". Se omitido, o padrão é "user").*
...
> **Importante:** Todas as rotas abaixo exigem o Header:
...
```json
{
  ...
  "descricao": "Verificar aulas de API",
  "prioridade": "alta",
  "status": "pendente"
}
...
*(Opções de Prioridade: baixa, media, alta)* *(Opções de Status: pendente, em_andamento, concluida)*

### Listar Tarefas

Retorna as tarefas do usuário logado.
*Se o usuário for Admin, retorna as tarefas de TODOS os usuários.*
...
> **Importante:** Exige Token Bearer. Um usuário não pode avaliar a si mesmo.

### Criar Avaliação

Registra uma nota e comentário para outro usuário.
...
```json
{
  "avaliado_id": 2,
  "nota": 5,
  "comentario": "Excelente profissional, entregou no prazo!"
}

```


*(Regra: A nota deve ser um número inteiro de 1 a 5)*

### Ver Relatório de Avaliações (Média)

Retorna o desempenho de um usuário específico, incluindo a média calculada automaticamente.

* **URL:** `/avaliacoes/recebidas/:usuarioId`
* **Exemplo:** `/avaliacoes/recebidas/2`
* **Método:** `GET`
* **Resposta Sucesso (200):**
```json
{
...
Sistema completo de backend integrando gestão de tarefas e sistema de avaliações de usuários, com autenticação JWT e controle de permissões.
